...
class myFSM:
    ...
    def printFSM(self):
        ...
        for key, value in self.states.items():
            print('Estado: %s' % key)
            for path in value:
                print('\t Evento: %s, Accion: %s,Sig Estado: %s' %
                      (path.event, path.callback.__name__, path.nextState))

    def whereAmI(self):
        '''Funcion para debuggear, indica el estado actual y las posibilidades.'''
        print("Estoy en el estado %s y mis posibles caminos son: " %
              self.currState)
    ...
    def run(self, event):
        ...
        if self.currState == None:
            # No es key error, pero no se que poner
            raise KeyError("No hay estado inicial setteado.")
        ...
        if invalidEvent:
            raise KeyError(
                "Dicho evento no esta asociado al estado actual.")  # Same
